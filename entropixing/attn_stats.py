...
class AttnStats(NamedTuple):
    ...
    @classmethod
    def init(self, bsz: int, n_layers: int, n_heads: int, device: torch.device):
        return self(
            entropy=torch.zeros((bsz, n_layers, n_heads), device=device),
            varentropy=torch.zeros((bsz, n_layers, n_heads), device=device),
            n_layers=n_layers,
            n_heads=n_heads,
        )
    ...
    def update(self, scores: torch.Tensor, layer_idx: int):
        ...
        new_entropy = -torch.sum(
            torch.where(probs > 0, probs * torch.log(probs), torch.tensor(0.0)), dim=-1
        )
        new_varentropy = torch.sum(
            probs * (torch.log(probs) + new_entropy.unsqueeze(-1)) ** 2, dim=-1
        )
